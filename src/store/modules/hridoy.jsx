...
Mock.onPost("/api/scrum-board/add-member").reply(config => {
  const { boardId, memberId } = JSON.parse(config.data);
...
Mock.onPost("/api/scrum-board/add-column").reply(config => {
  const { boardId, listTitle } = JSON.parse(config.data);
...
Mock.onPost("/api/scrum-board/rename-column").reply(config => {
  const { boardId, listTitle, listId } = JSON.parse(config.data);
...
Mock.onPost("/api/scrum-board/delete-column").reply(config => {
  const { boardId, listId } = JSON.parse(config.data);
...
Mock.onPost("/api/scrum-board/add-card").reply(config => {
  const { boardId, cardTitle, listId } = JSON.parse(config.data);

  const board = ScrumBoardDB.boardList.find(board => board.id === boardId);
  const column = board.list.find(column => column.id === listId);
...
Mock.onPost("/api/scrum-board/update-card").reply(config => {
  const { boardId, listId, card } = JSON.parse(config.data);

  const boardList = ScrumBoardDB.boardList.map(board => {
    if (board.id === boardId) {
      return {
        ...board,
...
Mock.onPost("/api/scrum-board/reorder-list").reply(config => {
  const { boardId, startIndex, endIndex } = JSON.parse(config.data);
  ...
  const board = ScrumBoardDB.boardList.find(item => item.id === boardId);
  const response = getBoardWithMemberDetails(board);
...
Mock.onPost("/api/scrum-board/reorder-card").reply(config => {
  const { boardId, listId, startIndex, endIndex } = JSON.parse(config.data);
